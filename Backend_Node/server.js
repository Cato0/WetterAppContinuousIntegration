...
function getTodaysWeatherData (cityName) {	
	...
	return new Promise ( (resolve, reject) => {
		...
		http.get('http://api.openweathermap.org/data/2.5/weather?q=' + cityName + '&appid=2edbe7c19ccf47195df9c1edfe3cc98e', (resp) => {
			...
			let wItem = new weatherItem(cityName, Number((kelvinToC(item.main.temp)).toFixed(1)), getDayOfWeek(new Date()), Number(kelvinToC(item.main.temp_min).toFixed(1)), Number(kelvinToC(item.main.temp_max)).toFixed(1), item.main.humidity, item.wind.speed, new Date(), item.weather[0].description);
	...
	return new Promise ( (resolve, reject) => {
		...
		http.get('http://api.openweathermap.org/data/2.5/forecast?q=' + cityName + ',DE&appid=2edbe7c19ccf47195df9c1edfe3cc98e', (resp) => {
			...
			resp.on('end', () => {
				...
				var wItem1 = new weatherItem(cityName, kelvinToC(item.main.temp).toFixed(1), getDayOfWeek(item.dt_txt), kelvinToC(item.main.temp_min).toFixed(1), kelvinToC(item.main.temp_max).toFixed(1), item.main.humidity, (item.wind.speed).toFixed(1), item.dt_txt, item.weather[0].description);
